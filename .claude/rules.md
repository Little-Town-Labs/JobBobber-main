...
export const agentNegotiation = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-notifications", async () => {
      ...
      await Promise.all([notifySeeker(seekerId, match), notifyEmployer(employerId, match)])
    })
...
const prompt = variant === "variant-a" ? EMPLOYER_AGENT_PROMPT_V1 : EMPLOYER_AGENT_PROMPT_V2
```
